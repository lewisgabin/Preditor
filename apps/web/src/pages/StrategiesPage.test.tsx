...
it('shows twenty methods with source, destination, rule and version', async () => {
  ...
  const renderedCodes = screen.getAllByText(/^[PA]\d{2}$/, { selector: 'h3' }).map(heading => heading.textContent)
  for (const method of methods) expect(renderedCodes).toContain(method.code)
...
it('explains missing historical sources after generation finishes', async () => {
  vi.spyOn(globalThis, 'fetch').mockImplementation((input, init) => {
    const url = typeof input === 'string' ? input : input instanceof URL ? input.href : input.url
    if (url.includes('/auth/')) return Promise.resolve(json({ data: { name: 'Owner' } }))
    if (init?.method === 'POST') return Promise.resolve(json({ data: { generated: 0, already_exists: 0, missing_source: 20, timing_blocked: 0, error: 0, signals: [] } }))
    return Promise.resolve(json({ data: [] }))
  })
  mount('signals')
  expect(await screen.findByText('Sin señales para esta fecha')).toBeInTheDocument()
  expect(screen.getByText(/No se encontraron fuentes suficientes/)).toBeInTheDocument()
})
it('automatically generates for the selected date and invalidates the list to show the backend calculation', async () => {
  ...
  expect(await screen.findByText('Acertó en segunda y tercera.')).toBeInTheDocument()
  expect(screen.getByLabelText('Segunda: 07, acierto')).toBeInTheDocument()
  expect(screen.getByLabelText('Tercera: 07, acierto')).toBeInTheDocument()
